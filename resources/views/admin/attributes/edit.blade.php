...
<div class="p-6 max-w-2xl mx-auto">
  <h1 class="text-2xl font-semibold mb-4 text-gray-800">Sửa thuộc tính: {{ $attribute->name }}</h1>

  <form action="{{ route('admin.attribute.update', $attribute) }}" method="POST" class="space-y-5 bg-white p-6 rounded-lg shadow">
    @csrf
    @method('PUT')
    ...
    <div class="flex gap-4 mt-4">
      <label class="flex items-center space-x-2">
        <input type="checkbox" name="is_filterable" value="1" {{ $attribute->is_filterable ? 'checked' : '' }} class="rounded text-blue-600">
        <span class="text-gray-700 text-sm">Hiển thị trong bộ lọc</span>
      </label>
      <label class="flex items-center space-x-2">
        <input type="checkbox" name="is_variation" value="1" {{ $attribute->is_variation ? 'checked' : '' }} class="rounded text-blue-600">
