...
<div class="max-w-3xl mx-auto p-8">
  ...
  <div class="mb-6 flex items-center justify-between">
    <div>
      <h1 class="text-3xl font-bold text-gray-800 tracking-tight">✨ Thêm giá trị thuộc tính</h1>
      <p class="text-gray-500 mt-1 text-sm">Tạo giá trị cho thuộc tính sản phẩm (ví dụ: Màu sắc, Kích thước, v.v.)</p>
    </div>
  ...
  {{-- Form --}}
  <form action="{{ route('admin.attribute_value.store') }}" method="POST"
    ...
    <div>
      ...
      <div class="relative">
        ...
        <input type="text" name="value" value="{{ old('value') }}"
               class="w-full h-11 pl-9 border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 bg-gray-50"
               placeholder="Ví dụ: red, XL..." required>
      </div>
      @error('value')
        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
      @enderror
    ...
    <div class="flex justify-end pt-4 border-t border-gray-100">
      ...
      <button type="submit"
              class="ml-3 inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-medium rounded-lg shadow hover:shadow-md hover:scale-[1.02] transition-all duration-200">
